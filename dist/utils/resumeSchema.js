...
exports.normalizeResumeInput = normalizeResumeInput;
exports.validateResumeInput = validateResumeInput;
exports.createResumeViewModel = createResumeViewModel;
...
function normalizeResumeInput(input) {
    ...
    return {
        ...
        experience: normalizeArray(safe.experience)
            .map((item) => ({
            ...
            highlights: compact(normalizeArray(item.highlights).map(normalizeString)),
        }))
            .filter((item) => item.role || item.company || item.highlights.length || item.startDate || item.endDate),
        education: normalizeArray(safe.education)
            .map((item) => ({
            id: normalizeString(item.id),
        ...
        skills: normalizeArray(safe.skills)
            .map((item) => ({
            id: normalizeString(item.id),
        ...
        projects: normalizeArray(safe.projects)
            .map((item) => ({
            id: normalizeString(item.id),
            name: normalizeString(item.name),
            role: normalizeString(item.role),
        ...
        certifications: normalizeArray(safe.certifications)
            .map((item) => ({
            id: normalizeString(item.id),
            name: normalizeString(item.name),
        ...
        languages: normalizeArray(safe.languages)
            .map((item) => ({
            id: normalizeString(item.id),
            name: normalizeString(item.name),
...
function validateResumeInput(input) {
    ...
    if (!/\S+@\S+\.\S+/.test(input.profile.email))
        return { valid: false, error: "Invalid email format" };
    if (!input.profile.phone)
        return { valid: false, error: "Phone number is required" };
    if (!input.profile.summary)
        return { valid: false, error: "Professional summary is required" };
    if (!input.experience.length)
        return { valid: false, error: "At least one experience entry is required" };
    if (!input.education.length)
        return { valid: false, error: "At least one education entry is required" };
...
function createResumeViewModel(input) {
